...
(function RSVPExercise() {
  ...
  /* ── WPM presets per difficulty ──────────────────────────── */
  const WPM_PRESETS = { easy: 150, medium: 300, hard: 500 };
  ...
  document.querySelectorAll('.diff-btn[data-level]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      wpm      = WPM_PRESETS[btn.dataset.level] || 150;
      startWpm = wpm;
      slider.value         = wpm;
  ...
  function loadPassage() {
    ...
    const text = PASSAGES[currentPassageIdx];
  ...
  /* ══════════════════════════════════════════════════════════
     WORD RENDERING — Optimal Recognition Point (ORP)
     Research shows the eye anchors ~30% into a word.
     Highlighting that letter trains faster fixation.
     ══════════════════════════════════════════════════════════ */
  ...
  function scheduleNext() {
    ...
    /* Pause longer on punctuation so meaning can register */
    const hasPause = /[.,?!;:]$/.test(word);
    const delay    = hasPause ? baseMs * 2.2 : baseMs;

    renderWord(word);
    if (fillEl) fillEl.style.width = ((idx / words.length) * 100).toFixed(1) + '%';
    idx++;

    /* Auto speed increase every 10 words */
    if (autoIncrease && idx % 10 === 0) {
    ...
    function renderQuestion() {
      ...
      stage.querySelectorAll('.comp-opt').forEach(btn => {
        btn.addEventListener('click', () => {
          ...
          const allOpts  = stage.querySelectorAll('.comp-opt');
          ...
          if (selected === q.answer) {
            ...
            allOpts[q.answer].style.background  = 'rgba(6,255,165,0.1)';
            allOpts[q.answer].style.borderColor = 'var(--green)';
          }
  ...
  function renderFinalStats(peakWpm, totalQ) {
    ...
    const msg = accuracy === 100 ? 'Perfect comprehension.'
              : accuracy >= 50   ? 'Good focus. Push the speed next time.'
              :                    'Slow down slightly to improve comprehension.';
    ...
    document.getElementById('rsvp-restart-btn').addEventListener('click', () => {
      ...
      loadPassage();
      playBtn.textContent  = '▶ Start';
      /* Reload the exercise HTML shell without page refresh */
